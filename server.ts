...
async function loadUsersWithMetadata(): Promise<{ users: DashboardUser[] }> {
  ...
  if (isConnected && db) {
    ...
    try {
      await cleanupExpiredTemporaryUsers(uri, db);
    } catch (cleanupError) {
    ...
    try {
      ...
      tempUsersMeta = tempUsers.map((t) => ({ username: t.username, expiresAt: t.expiresAt }));
    } catch (metaError) {
      // eslint-disable-next-line no-console
...
app.post('/validate-uri', async (req: Request, res: Response): Promise<void> => {
  try {
...
app.post('/connect', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (uri.includes('demo') || uri.includes('test-demo')) {
      ...
      res.json({ success: true, message: 'Connected to demo database successfully' });
      return;
    }
...
app.post('/disconnect', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Failed to disconnect from MongoDB: ' + message });
  }
});
...
function validatePasswordCharacters(password: string): void {
  ...
  if (!/^[A-Za-z0-9]+$/.test(password)) {
    throw new Error('Password may only contain letters and numbers (no special characters)');
  }
}
...
app.post('/users', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (demoMode) {
      res.json({ success: true, message: 'User created successfully (demo mode)' });
      return;
    }
    ...
    const { name, password, roles = [] } = req.body as {
      name?: string;
      password?: string;
      roles?: InheritedRole[];
    };
...
app.post('/users/temporary', async (req: Request, res: Response): Promise<void> => {
  try {
    if (!isConnected && !demoMode) {
      res.status(400).json({ error: 'Not connected to database' });
      return;
    ...
    const { name, password, roles = [], expiresInHours } = req.body as {
      name?: string;
      password?: string;
      roles?: InheritedRole[];
    ...
    if (demoMode) {
      res.json({ success: true, message: 'Temporary user created successfully (demo mode)' });
      return;
    }
    ...
    res.json({ success: true, message: 'Temporary user created successfully' });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('Error creating temporary user:', error);
    const message = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Failed to create temporary user: ' + message });
  }
});
...
// Delete user
app.delete('/users/delete', async (req: Request, res: Response): Promise<void> => {
  try {
    if (!isConnected && !demoMode) {
      res.status(400).json({ error: 'Not connected to database' });
      return;
    ...
    if (demoMode) {
      ...
      if (remainingUsers.length === 0) {
        res.status(400).json({
          error: 'Cannot delete the last user. At least one user must remain to access the database.'
        });
        return;
      }
      ...
      if (
        ...
        res.status(400).json({
          error: 'Root users and users with administrative privileges cannot be deleted for security reasons.'
        });
        return;
      }

      res.json({ success: true, message: 'User deleted successfully (demo mode)' });
      return;
    }
...
app.get('/users/:name/effective-privileges', async (req: Request, res: Response): Promise<void> => {
  try {
    if (!isConnected && !demoMode) {
      res.status(400).json({ error: 'Not connected to database' });
      return;
    ...
    const message = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Failed to get effective privileges: ' + message });
  }
});
...
// Create custom role
app.post('/roles/custom', async (req: Request, res: Response): Promise<void> => {
  try {
    if (!isConnected && !demoMode) {
      res.status(400).json({ error: 'Not connected to database' });
      return;
    ...
    const { roleName, privileges = [], inheritedRoles = [] } = req.body as {
      roleName?: string;
      privileges?: unknown[];
    ...
    if (demoMode) {
      res.json({ success: true, message: 'Custom role created successfully (demo mode)' });
      return;
    }
    ...
    console.log('[roles/custom] create request:', {
      ...
      inheritedRolesCount: Array.isArray(inheritedRoles) ? inheritedRoles.length : 0,
    });
    ...
    const message = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Failed to create custom role: ' + message });
  }
});

// Update custom role
app.put('/roles/custom/:roleName', async (req: Request, res: Response): Promise<void> => {
  try {
    if (!isConnected && !demoMode) {
      res.status(400).json({ error: 'Not connected to database' });
      return;
    ...
    if (demoMode) {
      res.json({ success: true, message: 'Custom role updated successfully (demo mode)' });
      return;
    }
    ...
    const message = error instanceof Error ? error.message : String(error);
    res.status(500).json({ error: 'Failed to update custom role: ' + message });
  }
});

// Delete custom role
app.delete('/roles/custom/:roleName', async (req: Request, res: Response): Promise<void> => {
  try {
    if (!isConnected && !demoMode) {
      res.status(400).json({ error: 'Not connected to database' });
      return;
    ...
    if (demoMode) {
      res.json({ success: true, message: 'Custom role deleted successfully (demo mode)' });
      return;
    }
    ...
    res.status(statusCode).json({ error: 'Failed to delete custom role: ' + message });
  }
});
